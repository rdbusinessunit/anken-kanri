...
function docHearing(c, p, i) {
  ...
  const rows = [
    ...
    ...[0, 1, 2, 3].map(j => TR(j === 0 ? H('勤務時間', 2) : TD('', 2, 'h'), V(marks[j], 1, 'c'), V(sh[j] ? shiftText(sh[j]) : '', 4, 'c'), H('備考', 1), V(sh[j] ? `${sh[j].label}（実働${fmtH(shiftH(sh[j]))}H）${sh[j].note ? '　' + sh[j].note : ''}` : '', 4))),
...
function adDraft(c, p) {
  ...
  return [
    ...
    `【勤務時間】${p.shifts.filter(s => s.start).map(s => `${s.label} ${shiftText(s)}（休憩${s.breakMin}分）${s.note ? '／' + s.note : ''}`).join('／')}${allShiftNotes(p) ? '\n　' + allShiftNotes(p) : ''}`,
